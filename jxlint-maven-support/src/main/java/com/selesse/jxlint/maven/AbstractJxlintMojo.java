...
public abstract class AbstractJxlintMojo extends AbstractMojo {
  ...
  private void addRulesOption(ProgramOptions options, JxlintOption option, List<String> list) throws MojoExecutionException {
  ...
  private void addCategoryOption(ProgramOptions options, List<String> rawCategoryStringList) throws MojoExecutionException {
    if (rawCategoryStringList != null && !rawCategoryStringList.isEmpty()) {
      ...
      for (String categoryString : rawCategoryStringList) {
        if (!categoryNames.contains(categoryString)) {
          throw new MojoExecutionException("Category \"" + categoryString + "\" does not exist. Try one of: " +
